...
class SyncContacts extends Command
{
     ...
     * @var string
     */
    protected $signature = 'hotspot:sync-contacts';
    ...
    public function handle()
    {
        ...
        foreach ($response->getResults() as $val) {
            ...
            $result = Contact::firstOrCreate(
                ...
                [
                    ...
                    'contact_status_id' => $contact['hs_content_membership_status'] == 'active' ? 1 : 0,
        ...
        if (isset($alreadyExists)) {
            echo "These records already exists : " . implode(", ", $alreadyExists) . ". $count new records added.";
        } else {
            echo "$count new records added.";
        }
